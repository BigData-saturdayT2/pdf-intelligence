...
# Updated Function to send the extracted text, question, and custom prompt to FastAPI
...
actual_answer = "PLACE HOLDER FOR THE ACTUAL ANSWER"
steps = """
...
def question_answering_page():
    ...
    # Dropdown menu for selecting a question prompt
    ...
    if selected_prompt and selected_prompt != "Select a prompt":
        ...
        if extracted_text:
            ...
            if st.button("Run Prompt"):
                ...
                st.info(f"Sending the following data with corresponding extracted text to OpenAI:\n**Question**: {selected_prompt}\n**Custom Prompt**: {user_prompt}")
                try:
                    openai_response = process_openai_query(
                        extracted_text=extracted_text,
                        question=selected_prompt,
                        prompt=user_prompt
                    )
...
# Entry point for the question answering page
